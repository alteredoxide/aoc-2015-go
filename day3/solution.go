...
func part2(input string) int {
    santa_coords := [2]int{0, 0}
    robo_coords := [2]int{0, 0}
    current_coords := [2][2]int{santa_coords, robo_coords}
    ...
    for i, c := range input {
        ...
        switch c {
        case '^':
            current_coords[agent][0] += 1
        case 'v':
            current_coords[agent][0] -= 1
        case '>':
            current_coords[agent][1] += 1
        case '<':
            current_coords[agent][1] -= 1
        }
        coords[current_coords[agent]] = true
    }
    return len(coords)
}
